(define (domain domini_ext2)
    ...
    (:functions
        ...
        (combustible-gastado ?r - rover) ; et dona la quantitat de combuustible que queda al diposit del rover
        (capacidad-combustible) ; retorna la capacitat que te el diposit de combustible
    )
    ...
    (:action mover_rover
        ...
        :precondition (and (estacionado ?r ?bOri) (adjacente ?bOri ?bDest) (> (- (capacidad-combustible) (combustible-gastado ?r)) 0)) ; el rover esta estacionat a la baseOri i la baseDest es adjacent. Hi ha combustible disponible
        :effect (and (estacionado ?r ?bDest) (not(estacionado ?r ?bOri)) (increase (combustible-gastado ?r) 1)) ; el rover NO esta estacionat a l'origen i ho està a la baseDest. Es decrementa la qtt de combustible
    ) 
)